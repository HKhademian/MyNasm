...
"%bindir%\GoLink.exe" /console /entry main /fo "%outdir%\%fileName%.exe" /ni "%outdir%\%fileName%.obj" "%bindir%\libw.obj" kernel32.dll  ucrtbase.dll

REM [UN]COMMENT TO delete .obj file
DEL "%outdir%\%fileName%.obj" >nul 2>&1

IF NOT EXIST "%outdir%\%fileName%.exe" GOTO ExeNotFound
